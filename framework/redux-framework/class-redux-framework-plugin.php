...
if ( ! class_exists( 'Redux_Framework_Plugin', false ) ) {
	...
	class Redux_Framework_Plugin {
		...
		private static function single_activate() {
			...
			$notices[] = esc_html__( 'Redux Framework has an embedded demo.', 'wpoven-performance-logs' ) . ' <a href="./plugins.php?redux-framework-plugin=demo&nonce=' . $nonce . '">' . esc_html__( 'Click here to activate the sample config file.', 'wpoven-performance-logs' ) . '</a>';
		...
		public function plugin_metalinks( array $links, string $file ): array {
			if ( strpos( $file, 'redux-framework.php' ) !== false && is_plugin_active( $file ) ) {
				$links[] = '<a href="' . esc_url( admin_url( add_query_arg( array( 'page' => 'redux-framework' ), 'options-general.php' ) ) ) . '">' . esc_html__( 'What is this?', 'wpoven-performance-logs' ) . '</a>';
			}
