	...
	class Redux_Core {
		...
		private function includes() {
			if ( is_admin() ) {
				if ( class_exists( 'Redux_Pro' ) && isset( Redux_Pro::$dir ) ) {
					echo '<div class="error"><p>' . sprintf( esc_html__( 'Redux has detected the Redux Pro plugin is enabled. All featured of Redux Pro are now part of the entire Redux plugin and is no longer required. Please disable the Redux Pro plugin to avoid potential conflicts.', 'wpoven-performance-logs' ), '<code></code>' ) . '</p></div>';
				}
			}
		...
		public function add_debug_info( array $debug_info ): array {
			...
			$debug_info['wp-filesystem']['fields'][] = array(
				'label' => esc_html__( 'The Redux upload directory', 'wpoven-performance-logs' ),
			...
			$debug_info['redux-framework'] = array(
				'label'       => esc_html__( 'Redux Framework', 'wpoven-performance-logs' ),
				'description' => esc_html__( 'Debug information specific to Redux Framework.', 'wpoven-performance-logs' ),
				'fields'      => array(
					'version'        => array(
						'label' => esc_html__( 'Version', 'wpoven-performance-logs' ),
						'value' => self::$version,
					),
					'installation'   => array(
						'label' => esc_html__( 'Installation', 'wpoven-performance-logs' ),
						'value' => self::$installed,
					),
					'data directory' => array(
						'label' => esc_html__( 'Data directory', 'wpoven-performance-logs' ),
						'value' => self::$dir,
					),
					'browser'        => array(
						'label' => esc_html__( 'Browser', 'wpoven-performance-logs' ),
						'value' => $browser_data,
					),
				),
			);
			...
			if ( ! empty( $redux ) && is_array( $redux ) ) {
				foreach ( $redux as $inst => $data ) {
					...
					$debug_info[ 'redux-instance-' . $inst ] = array(
						...
						'label'       => sprintf( esc_html__( 'Redux Instance: %s', 'wpoven-performance-logs' ), $inst_name ),
						// translators: %s = Instance name w/ HTML.
						'description' => sprintf( esc_html__( 'Debug information for the %s Redux instance.', 'wpoven-performance-logs' ), '<code>' . $inst . '</code>' ),
						'fields'      => array(
							'opt_name'         => array(
