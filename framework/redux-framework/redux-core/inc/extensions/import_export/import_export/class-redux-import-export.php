...
if ( ! class_exists( 'Redux_Import_Export', false ) ) {
	...
	class Redux_Import_Export extends Redux_Field {
		...
		public function render() {
			...
			?>
			<h4><?php esc_html_e( 'Import Options', 'wpoven-performance-logs' ); ?></h4>
			<p>
				<a
					href="javascript:void(0);"
					id="redux-import-code-button"
					class="button-secondary">
					<?php esc_html_e( 'Import from Clipboard', 'wpoven-performance-logs' ); ?>
				</a>
				<a
					href="#"
					id="redux-import-upload"
					class="button-secondary">
					<?php esc_html_e( 'Upload file', 'wpoven-performance-logs' ); ?><span></span>
				</a>
			...
			<div id="redux-import-code-wrapper">
				<p class="description" id="import-code-description">
					...
					<?php echo esc_html( apply_filters( 'redux-import-file-description', esc_html__( 'Paste your clipboard data here.', 'wpoven-performance-logs' ) ) ); ?>
				</p>
			...
			<p id="redux-import-action">
				<input
					...
					value="<?php esc_html_e( 'Import', 'wpoven-performance-logs' ); ?>">&nbsp;&nbsp;
				<span>
					<?php // phpcs:ignore WordPress.NamingConventions.ValidHookName ?>
					<?php echo esc_html( apply_filters( 'redux-import-warning', esc_html__( 'WARNING! This will overwrite all existing option values, please proceed with caution!', 'wpoven-performance-logs' ) ) ); ?>
				</span>
			</p>
			<div class="hr">
				<div class="inner">
			...
			<h4><?php esc_html_e( 'Export Options', 'wpoven-performance-logs' ); ?></h4>
			<div class="redux-section-desc">
				<p class="description">
					<?php // phpcs:ignore WordPress.NamingConventions.ValidHookName ?>
					<?php echo esc_html( apply_filters( 'redux-backup-description', esc_html__( 'Here you can copy/download your current option settings. Keep this safe as you can use it as a backup should anything go wrong, or you can use it to restore your settings on this site (or any other site).', 'wpoven-performance-logs' ) ) ); ?>
				</p>
			</div>
			...
			<p>
				<button id="redux-export-code-copy" class="button-secondary"
						...
						data-copy="<?php esc_attr_e( 'Copy to Clipboard', 'wpoven-performance-logs' ); ?>"
						data-copied="<?php esc_attr_e( 'Copied!', 'wpoven-performance-logs' ); ?>">
					<?php esc_html_e( 'Copy to Clipboard', 'wpoven-performance-logs' ); ?>
				</button>
				<a href="<?php echo esc_url( $link ); ?>" id="redux-export-code-dl" class="button-primary">
					<?php esc_html_e( 'Export File', 'wpoven-performance-logs' ); ?>
				</a>
			</p>
			<p></p>
		...
		public function enqueue() {
			...
			wp_localize_script(
				...
				'ImportExport',
				array(
					'unchanged_values' => esc_html__( 'Your panel has unchanged values, would you like to save them now?', 'wpoven-performance-logs' ),
				)
			);
