					...
					foreach ( $fonts as $section ) {
						if ( 'd' === $section['type'] && ! empty( $section['name'] ) ) {
							if ( 'custom' === $section['name'] ) {
								$section['name'] = esc_html__( 'Custom Fonts', 'wpoven-performance-logs' );
							}
		...
		public function ajax() {
			...
			if ( isset( $_POST['type'] ) && 'delete' === $_POST['type'] ) {
				if ( isset( $_POST['section'] ) ) {
					if ( esc_html__( 'Custom Fonts', 'wpoven-performance-logs' ) === $_POST['section'] ) {
				...
				try {
					...
					echo wp_json_encode(
						array(
							'type' => 'error',
							'msg'  => esc_html__( 'Unable to delete font file(s).', 'wpoven-performance-logs' ),
						)
					);
				}
		...
		public function process_web_font( string $attachment_id, string $name, string $true_filename, string $mime_type ) {
			...
			if ( empty( $path ) ) {
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'Attachment does not exist.', 'wpoven-performance-logs' ),
					)
				);
			...
			if ( 'zip' === $subtype ) {
				...
				if ( is_wp_error( $unzipfile ) ) {
					echo wp_json_encode(
						array(
							'type' => 'error',
							'msg'  => $unzipfile->get_error_message() . '<br><br>' . esc_html__( 'Unzipping failed.', 'wpoven-performance-logs' ),
						)
					);
				...
				echo wp_json_encode(
					array(
						'type' => 'error',
						'msg'  => esc_html__( 'File type not recognized.', 'wpoven-performance-logs' ) . ' ' . $subtype,
					)
				);
			...
			if ( is_array( $ret ) && ! empty( $ret ) ) {
				$msg = esc_html__( 'Unidentified error.', 'wpoven-performance-logs' );
			...
			if ( ! empty( $name ) && ! empty( $missing ) ) {
				...
				if ( ! isset( $main ) ) {
					echo wp_json_encode(
						array(
							'type' => 'error',
							'msg'  => esc_html__( 'No valid font file was found.', 'wpoven-performance-logs' ),
						)
					);
				...
				if ( is_wp_error( $response ) ) {
					return array(
						'type' => 'error',
						'msg'  => $response->get_error_message() . '<br><br>' . esc_html__( 'Your font could not be converted at this time. Please try again later.', 'wpoven-performance-logs' ),
					);
				...
				if ( ! is_wp_error( $zip ) ) {
					...
					if ( in_array( $font_ext, $unsupported, true ) ) {
						return array(
							...
							'msg'  => $zip->get_error_message() . '<br><br>' . sprintf( esc_html__( 'The font converter does not support %s fonts.', 'wpoven-performance-logs' ), $font_ext ),
						);
					} else {
						return array(
							'type' => 'error',
							'msg'  => $zip->get_error_message() . '<br><br>' . esc_html__( 'ZIP error. Your font could not be converted at this time. Please try again later.', 'wpoven-performance-logs' ),
						);
					}
				}
		...
		public function add_section() {
			if ( ! isset( $this->parent->fontControl ) ) {
				$this->parent->sections[] = array(
					'title'  => esc_html__( 'Font Control', 'wpoven-performance-logs' ),
				...
				for ( $i = count( $this->parent->sections ); $i >= 1; $i-- ) {
					if ( isset( $this->parent->sections[ $i ] ) && isset( $this->parent->sections[ $i ]['title'] ) && esc_html__( 'Font Control', 'wpoven-performance-logs' ) === $this->parent->sections[ $i ]['title'] ) {
