		...
		public function render() {
			...
			if ( ! empty( $this->field['custom_fonts'] ) ) {
				foreach ( $this->field['custom_fonts'] as $section => $fonts ) {
					...
					echo '<div class="font-error" style="display: none;"><p><strong>' . esc_html__( 'Error', 'wpoven-performance-logs' ) . '</strong>: <span></span></p></div>';
					...
					foreach ( $fonts as $font => $pieces ) {
						...
						echo '<a href="#" class="fontDelete delete" data-section="' . esc_attr( $section ) . '" data-name="' . esc_attr( $font ) . '" data-type="delete">' . esc_html__( 'Delete', 'wpoven-performance-logs' ) . '</a>';
				...
				echo '<div class="upload_button_div"><span class="button media_add_font" data-nonce="' . esc_attr( $nonce ) . '" id="' . esc_attr( $this->field['id'] ) . '-custom_fonts">' . esc_html__( 'Add Font', 'wpoven-performance-logs' ) . '</span></div><br />';
			} else {
				echo '<h3>' . esc_html__( 'No Custom Fonts Found', 'wpoven-performance-logs' ) . '</h3>';
				echo '<div class="upload_button_div"><span class="button media_add_font" data-nonce="' . esc_attr( $nonce ) . '" id="' . esc_attr( $this->field['id'] ) . '-custom_fonts">' . esc_html__( 'Add Font', 'wpoven-performance-logs' ) . '</span></div>';
			}
		...
		public function enqueue() {
			...
			wp_localize_script(
				...
				apply_filters(
					...
					array(
						'delete_error' => esc_html__( 'There was an error deleting your font:', 'wpoven-performance-logs' ),
						'unzip'        => esc_html__( 'Unzipping archive and generating any missing font files.', 'wpoven-performance-logs' ),
						'convert'      => esc_html__( 'Converting font file(s)...', 'wpoven-performance-logs' ),
						'partial'      => esc_html__( 'The only file(s) imported were those uploaded.  Please refresh the page to continue (making note of any errors before doing so, please).', 'wpoven-performance-logs' ),
						'unknown'      => esc_html__( 'An unknown error occurred. Please try again.', 'wpoven-performance-logs' ),
						'complete'     => esc_html__( 'Conversion complete.  Refreshing page...', 'wpoven-performance-logs' ),
						'media_title'  => esc_html__( 'Choose Font file or ZIP of font files.', 'wpoven-performance-logs' ),
						'media_button' => esc_html__( 'Update', 'wpoven-performance-logs' ),
					)
				)
			);
