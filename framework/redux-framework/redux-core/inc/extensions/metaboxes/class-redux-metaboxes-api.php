	...
	class Redux_Metaboxes {
		...
		public static function set_box( string $opt_name = '', array $box = array() ) {
			...
			if ( ! empty( $opt_name ) && is_array( $box ) && ! empty( $box ) ) {
				...
				self::$errors[ $opt_name ]['box']['empty'] = esc_html__( 'Unable to create a box due an empty box array or the box variable passed was not an array.', 'wpoven-performance-logs' );
			}
		}
