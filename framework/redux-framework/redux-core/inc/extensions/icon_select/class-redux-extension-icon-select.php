	...
	class Redux_Extension_Icon_Select extends Redux_Extension_Abstract {
		...
		public function get_icons() {
			...
			if ( ! wp_verify_nonce( $nonce, 'redux_icon_nonce' ) ) {
				wp_send_json_error( array( 'error' => esc_html__( 'Error: Invalid nonce verification.', 'wpoven-performance-logs' ) ) );
			}
			...
			if ( ! empty( $icon_lists ) ) {
				...
				echo '<div class="redux-error-text">' . esc_html__( 'No data available.', 'wpoven-performance-logs' ) . '</div>';
			}
