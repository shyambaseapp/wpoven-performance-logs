			...
			foreach ( $this->field['tabs'] as $key => $tab ) {
				...
				foreach ( $tab['fields'] as $field ) {
					...
					if ( in_array( $field['type'], $unallowed, true ) ) {
						echo esc_html__( 'The', 'wpoven-performance-logs' ) . ' <code>' . esc_html( $field['type'] ) . '</code> ' . esc_html__( 'field is not supported within the Tabbed field.', 'wpoven-performance-logs' );
					} else {
