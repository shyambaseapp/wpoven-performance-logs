		...
		public function __construct( $redux, array $args ) {
			...
			$default = array(
				...
				'font_weights'                     => array(
					array(
						'id'   => '400',
						'name' => __( 'Regular 400', 'wpoven-performance-logs' ),
					),
					array(
						'id'   => '400italic',
						'name' => __( 'Regular 400 Italic', 'wpoven-performance-logs' ),
					),
					array(
						'id'   => '700',
						'name' => __( 'Bold 700', 'wpoven-performance-logs' ),
					),
					array(
						'id'   => '700italic',
						'name' => __( 'Bold 700 Italic', 'wpoven-performance-logs' ),
					),
				),
		...
		private function no_errors_please( array $args ): array {
			...
			if ( empty( $args['footer_credit'] ) ) {
				...
				/* translators: 1: Redux, 2: Link to plugin review */
					__( 'Enjoyed %1$s? Please leave us a %2$s rating. We really appreciate your support!', 'wpoven-performance-logs' ),
					'<strong>' . __( 'Redux', 'wpoven-performance-logs' ) . '</strong>',
					'<a href="https://wordpress.org/support/plugin/wpoven-performance-logs/reviews/?filter=5/#new-post" target="_blank">&#9733;&#9733;&#9733;&#9733;&#9733;</a>'
				);
				$args['footer_credit'] = '<span id="footer-thankyou">' . $footer_text . '</span>';
			}

			if ( empty( $args['menu_title'] ) ) {
				$args['menu_title'] = esc_html__( 'Options', 'wpoven-performance-logs' );
			}

			if ( empty( $args['page_title'] ) ) {
				$args['page_title'] = esc_html__( 'Options', 'wpoven-performance-logs' );
			}
