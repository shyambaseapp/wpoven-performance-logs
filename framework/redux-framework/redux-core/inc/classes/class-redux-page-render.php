...
if ( ! class_exists( 'Redux_Page_Render', false ) ) {
	...
	class Redux_Page_Render extends Redux_Class {
		...
		/**
		 * Flag to show or hide hints in a panel.
		 *
		 * @var bool
		 * @access private
		 */
		...
		public function load_page() {
			...
			if ( true === $this->show_hints ) {
				...
				$dismiss = 'true';
				$s       = esc_html__( 'Enable', 'wpoven-performance-logs' );

				// Values for disabling hints.
				if ( 'true' === $hint_status ) {
					$dismiss = 'false';
					$s       = esc_html__( 'Disable', 'wpoven-performance-logs' );
				}
				...
				$event = esc_html__( 'moving the mouse over', 'wpoven-performance-logs' );
				if ( 'click' === $core->args['hints']['tip_effect']['show']['event'] ) {
					$event = esc_html__( 'clicking', 'wpoven-performance-logs' );
				}

				// Construct message.
				// translators: %1$s: Mouse action.  %2$s: Hint status.
				$msg = sprintf( esc_html__( 'Hints are tooltips that popup when %1$s the hint icon, offering addition information about the field in which they appear.  They can be %2$s by using the link below.', 'wpoven-performance-logs' ), $event, Redux_Core::strtolower( $s ) ) . '<br/><br/>' . $url;

				// Construct hint tab.
				$tab = array(
					'id'      => 'redux-hint-tab',
					'title'   => esc_html__( 'Hints', 'wpoven-performance-logs' ),
					'content' => '<p>' . $msg . '</p>',
				);
			...
			if ( '' !== $core->args['help_sidebar'] ) {
				...
				$screen->set_help_sidebar( '<p><strong>Redux Framework</strong><br/><br/>' . esc_html__( 'Hint Tooltip Preferences', 'wpoven-performance-logs' ) . '</p>' );
			}
				...
				if ( 'settings.php' !== $page_parent ) {
					...
					if ( $needle_pos > 0 ) {
						...
						if ( post_type_exists( $post_type ) ) {
							// Set the flag to add the menu page.
							$add_menu = true;
						}
						// custom menu.
					...
					// The page_parent was settings.php, so set menu add flag to true.
					$add_menu = true;
				}
		...
		public function field_input( array $field, $v = null ) {
			...
			if ( isset( $field['type'] ) ) {
				 ...
				 *
				 * @param string $path Field class file path.
				 * @param array  $field field data
				 */
				...
				if ( $class_file ) {
					...
					if ( ! class_exists( $field_class ) ) {
						if ( file_exists( $class_file ) ) {
							...
							printf( esc_html__( 'Field %1$s could not be displayed. Field type %2$s was not found.', 'wpoven-performance-logs' ), '<code>' . esc_attr( $field['id'] ) . '</code>', '<code>' . esc_attr( $field['type'] ) . '</code>' );
						}
					}
				}
		...
		/**
		 * Return default output string for use in a panel
		 *
		 * @param array $field Field array.
		 *
		...
		private function get_default_output_string( array $field ): string {
			...
			if ( ! empty( $default_output ) ) {
				$default_output = esc_html__( 'Default', 'wpoven-performance-logs' ) . ': ' . substr( $default_output, 0, - 2 );
			}
