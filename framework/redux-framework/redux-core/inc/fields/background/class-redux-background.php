	...
	class Redux_Background extends Redux_Field {
		...
		public function render() {
			...
			if ( true === $this->field['background-repeat'] ) {
				$array = array(
					'no-repeat' => esc_html__( 'No Repeat', 'wpoven-performance-logs' ),
					'repeat'    => esc_html__( 'Repeat All', 'wpoven-performance-logs' ),
					'repeat-x'  => esc_html__( 'Repeat Horizontally', 'wpoven-performance-logs' ),
					'repeat-y'  => esc_html__( 'Repeat Vertically', 'wpoven-performance-logs' ),
					'inherit'   => esc_html__( 'Inherit', 'wpoven-performance-logs' ),
				);

				echo '<select id="' . esc_attr( $this->field['id'] ) . '-repeat-select" data-placeholder="' . esc_html__( 'Background Repeat', 'wpoven-performance-logs' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-repeat]" class="redux-select-item redux-background-input redux-background-repeat ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
				echo '<option></option>';
			...
			if ( true === $this->field['background-clip'] ) {
				$array = array(
					'inherit'     => esc_html__( 'Inherit', 'wpoven-performance-logs' ),
					'border-box'  => esc_html__( 'Border Box', 'wpoven-performance-logs' ),
					'content-box' => esc_html__( 'Content Box', 'wpoven-performance-logs' ),
					'padding-box' => esc_html__( 'Padding Box', 'wpoven-performance-logs' ),
				);

				echo '<select id="' . esc_attr( $this->field['id'] ) . '-clip-select" data-placeholder="' . esc_html__( 'Background Clip', 'wpoven-performance-logs' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-clip]" class="redux-select-item redux-background-input redux-background-clip ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
				echo '<option></option>';
			...
			if ( true === $this->field['background-origin'] ) {
				...
				echo '<select id="' . esc_attr( $this->field['id'] ) . '-origin-select" data-placeholder="' . esc_html__( 'Background Origin', 'wpoven-performance-logs' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-origin]" class="redux-select-item redux-background-input redux-background-origin ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
				echo '<option></option>';
			...
			if ( true === $this->field['background-size'] ) {
				$array = array(
					'inherit' => esc_html__( 'Inherit', 'wpoven-performance-logs' ),
					'cover'   => esc_html__( 'Cover', 'wpoven-performance-logs' ),
					'contain' => esc_html__( 'Contain', 'wpoven-performance-logs' ),
				);

				echo '<select id="' . esc_attr( $this->field['id'] ) . '-size-select" data-placeholder="' . esc_html__( 'Background Size', 'wpoven-performance-logs' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-size]" class="redux-select-item redux-background-input redux-background-size ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
				echo '<option></option>';
			...
			if ( true === $this->field['background-attachment'] ) {
				$array = array(
					'fixed'   => esc_html__( 'Fixed', 'wpoven-performance-logs' ),
					'scroll'  => esc_html__( 'Scroll', 'wpoven-performance-logs' ),
					'inherit' => esc_html__( 'Inherit', 'wpoven-performance-logs' ),
				);

				echo '<select id="' . esc_attr( $this->field['id'] ) . '-attachment-select" data-placeholder="' . esc_html__( 'Background Attachment', 'wpoven-performance-logs' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-attachment]" class="redux-select-item redux-background-input redux-background-attachment ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
				echo '<option></option>';
			...
			if ( true === $this->field['background-position'] ) {
				$array = array(
					'left top'      => esc_html__( 'Left Top', 'wpoven-performance-logs' ),
					'left center'   => esc_html__( 'Left center', 'wpoven-performance-logs' ),
					'left bottom'   => esc_html__( 'Left Bottom', 'wpoven-performance-logs' ),
					'center top'    => esc_html__( 'Center Top', 'wpoven-performance-logs' ),
					'center center' => esc_html__( 'Center Center', 'wpoven-performance-logs' ),
					'center bottom' => esc_html__( 'Center Bottom', 'wpoven-performance-logs' ),
					'right top'     => esc_html__( 'Right Top', 'wpoven-performance-logs' ),
					'right center'  => esc_html__( 'Right center', 'wpoven-performance-logs' ),
					'right bottom'  => esc_html__( 'Right Bottom', 'wpoven-performance-logs' ),
				);

				echo '<select id="' . esc_attr( $this->field['id'] ) . '-position-select" data-placeholder="' . esc_html__( 'Background Position', 'wpoven-performance-logs' ) . '" name="' . esc_attr( $this->field['name'] . $this->field['name_suffix'] ) . '[background-position]" class="redux-select-item redux-background-input redux-background-position ' . esc_attr( $this->field['class'] ) . '"' . esc_attr( $select2_data ) . '>';
				echo '<option></option>';
			...
			if ( $this->field['background-image'] ) {
				...
				$placeholder = $this->field['placeholder'] ?? esc_html__( 'No media selected', 'wpoven-performance-logs' );
				...
				echo '<span class="button redux-background-upload" id="' . esc_attr( $this->field['id'] ) . '-media">' . esc_html__( 'Upload', 'wpoven-performance-logs' ) . '</span>';
				...
				echo '<span class="button removeCSS redux-remove-background' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $this->field['id'] ) . '" rel="' . esc_attr( $this->field['id'] ) . '">' . esc_html__( 'Remove', 'wpoven-performance-logs' ) . '</span>';
