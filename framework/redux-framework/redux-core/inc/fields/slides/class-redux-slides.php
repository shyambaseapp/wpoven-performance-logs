	...
	class Redux_Slides extends Redux_Field {
		...
		public function set_defaults() {
			$defaults = array(
				...
				'content_title' => esc_html__( 'Slide', 'wpoven-performance-logs' ),
			);
		...
		public function render() {
			...
			echo '<div class="redux-slides-accordion" data-new-content-title="' . esc_attr( sprintf( __( 'New %s', 'wpoven-performance-logs' ), $this->field['content_title'] ) ) . '">';
			...
			if ( is_array( $this->value ) && ! empty( $this->value ) ) {
				...
				foreach ( $slides as $slide ) {
					...
					echo '<span class="button media_upload_button" id="add_' . esc_attr( $x ) . '">' . esc_html__( 'Upload', 'wpoven-performance-logs' ) . '</span>';
					...
					echo '<span
							...
							esc_html__( 'Remove', 'wpoven-performance-logs' ) . '</span>';
					...
					$placeholder = ( isset( $this->field['placeholder']['title'] ) ) ? esc_attr( $this->field['placeholder']['title'] ) : __( 'Title', 'wpoven-performance-logs' );
					echo '<li>';
					echo '<input
					...
					if ( $this->field['show']['description'] ) {
						$placeholder = ( isset( $this->field['placeholder']['description'] ) ) ? esc_attr( $this->field['placeholder']['description'] ) : __( 'Description', 'wpoven-performance-logs' );
						echo '<li>';
						echo '<textarea
					...
					$placeholder = ( isset( $this->field['placeholder']['url'] ) ) ? esc_attr( $this->field['placeholder']['url'] ) : __( 'URL', 'wpoven-performance-logs' );
					...
					echo '<li>';
					echo '<a href="javascript:void(0);" class="button deletion redux-slides-remove">' . esc_html__( 'Delete', 'wpoven-performance-logs' ) . '</a>';
					echo '</li>';
			...
			if ( 0 === $x ) {
				...
				echo '<span class="redux-slides-header">' . esc_html( sprintf( __( 'New %s', 'wpoven-performance-logs' ), esc_attr( $this->field['content_title'] ) ) ) . '</span>';
				echo '</h3>';
				echo '<div>';
				...
				echo '<span class="button remove-image' . esc_attr( $hide ) . '" id="reset_' . esc_attr( $x ) . '" rel="' . esc_attr( $this->parent->args['opt_name'] . '[' . $this->field['id'] ) . '][attachment_id]">' . esc_html__( 'Remove', 'wpoven-performance-logs' ) . '</span>';
			...
			echo '<a href="javascript:void(0);" class="button redux-slides-add button-primary" rel-id="' . esc_attr( $this->field['id'] ) . '-ul" rel-name="' . esc_attr( $this->field['name'] . '[title][]' . $this->field['name_suffix'] ) . '">' . esc_html( sprintf( __( 'Add %s', 'wpoven-performance-logs' ), esc_html( $this->field['content_title'] ) ) ) . '</a>';
			echo '<br/>';
		}
